...
def format_rating(rating_value: float | None, rating_count: int | None) -> str:
    ...
    stars = "★" * rounded_rating + "☆" * (5 - rounded_rating)
...
def format_perfume_card(perfume: PerfumeItem, index: int | None = None) -> str:
...
def format_store_offers(offers: list[StoreOffer]) -> str:
    ...
    for offer in offers[:5]:
        ...
        lines.append(f"<b>{escape(offer.store)}</b> — {price} {escape(offer.currency)}{old_price}")
