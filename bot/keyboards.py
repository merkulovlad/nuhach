...
def build_perfume_keyboard(
    ...
    if show_details:
        row1.append(
            InlineKeyboardButton(
                text="📋 Details",
                callback_data=f"details:{perfume.id}"
            )
        )
    if show_similar:
        row1.append(
            InlineKeyboardButton(
                text="🔄 Similar",
                callback_data=f"similar:{perfume.id}"
            )
        )
    if row1:
        buttons.append(row1)
    ...
    if show_reactions:
        row2 = [
            InlineKeyboardButton(
                text="👍",
                callback_data=f"like:{perfume.id}"
            ),
            InlineKeyboardButton(
                text="👎",
                callback_data=f"dislike:{perfume.id}"
            ),
            InlineKeyboardButton(
                text="💾 Save",
                callback_data=f"save:{perfume.id}"
            ),
        ]
        buttons.append(row2)
