...
class NormalizedIngestor:
    ...
    def ingest_brands(self, df: pd.DataFrame):
        ...
        brand_data: Dict[str, Tuple[Optional[str], Optional[str]]] = {}
        ...
        for _, row in df.iterrows():
            for col_en, col_ru in note_pairs:
                ...
                for i, note_en in enumerate(notes_en):
                    ...
                    if i < len(notes_ru) and notes_ru[i]:
                        if note_en_lower not in note_translations:
    ...
    def ingest_accords(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            for col_en, col_ru in accord_pairs:
                ...
                if accord_en and accord_ru:
                    if accord_en.lower() not in accord_translations:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Ingest perfume data into normalized PostgreSQL tables')
    parser.add_argument('--no-truncate', action='store_true', help='Skip truncating tables (append mode)')
